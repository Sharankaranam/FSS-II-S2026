with open('fuzzy.js', 'r') as f:
    content = f.read()

# Replace CATEGORY_CONFIGS
rules_json = """  restaurants: {
    key: 'restaurants',
    label: 'Restaurants',
...
score_record_function = """function scoreRecord(categoryKey, values, userInputs = {}) {
...
# Manual split replacements
old_configs = """  restaurants: {
    key: 'restaurants',
    label: 'Restaurants',
    parameters: ['Distance', 'Cost', 'Quality', 'Taste', 'Ambience'],
    baseWeights: { Distance: 0.15, Cost: 0.15, Quality: 0.25, Taste: 0.3, Ambience: 0.15 },
...
old_score_record = """function scoreRecord(categoryKey, values, userInputs = {}) {
  const config = CATEGORY_CONFIGS[categoryKey];
  const dynamicWeights = getDynamicWeights(categoryKey, userInputs);
  ...
  config.parameters.forEach((field) => {
    ...
    const weight = dynamicWeights[field];
    const contribution = (m.L * config.levelScores.L) +
      (m.M * config.levelScores.M) +
      (m.H * config.levelScores.H);

    // Aggregation: FinalScore = Priority * ServiceScore
    totalScore += contribution * weight;

    parameters.push({ name: field, score: rawVal, memberships: m });

    diagnosticRules.push({
      raw: `Contribution of ${field}`,
      output: contribution.toFixed(1),
...
content = content.replace(old_configs, rules_json)
content = content.replace(old_score_record, score_record_function)

with open('fuzzy.js', 'w') as f:
    f.write(content)
